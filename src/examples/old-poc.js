...
function createUIPanel() {
  ...
  // Main panel - much larger to fully accommodate all UI elements
  const panelGeometry = new THREE.PlaneGeometry(0.25, 0.3);  // Much larger panel size
  ...
  // Top-right corner handle - adjusted for extremely large panel
  const topRightHandle = new THREE.Mesh(handleGeometry, handleMaterial);
  topRightHandle.position.set(0.125 - handleSize/2, 0.15 - handleSize/2, 0.005); // Positioned for the much larger panel
  ...
  // Create Start Over button - only visible in final step
  appState.ui.startOverButton = createButton('Start Over', 0xFF0000);
  appState.ui.startOverButton.position.set(0, -0.1, 0.002);
  appState.ui.startOverButton.visible = false; // Initially hidden
  panelGroup.add(appState.ui.startOverButton);
...
function onSelectStart(event) {
  ...
  // Check if we hit the Start Over button (only visible in final step)
  if (appState.ui.startOverButton && appState.ui.startOverButton.visible) {
    const startOverIntersects = raycaster.intersectObject(appState.ui.startOverButton, true);
    if (startOverIntersects.length > 0) {
      // Visual feedback for button press
      const buttonMesh = appState.ui.startOverButton.children[0];
      buttonMesh.material.color.set(buttonMesh.userData.hoverColor);
      setTimeout(() => {
        buttonMesh.material.color.set(buttonMesh.userData.defaultColor);
      }, 200);
      
      // Complete reset - go back to step 1 and clear everything
      appState.currentStep = 0;
      
      // Remove all points and projections
      appState.wallPoints.forEach(point => scene.remove(point.mesh));
      appState.objectPoints.forEach(point => scene.remove(point.mesh));
      appState.anchorPoints.forEach(point => scene.remove(point.mesh));
...
function addAnchorPointsToProjection() {
  ...
  appState.anchorPoints.forEach((anchor, index) => {
    ...
    // Main circle - using bright green for mount points as requested
    const circle = new THREE.Mesh(
      new THREE.CircleGeometry(0.015, 32), // Slightly larger circle for better visibility
      new THREE.MeshBasicMaterial({ 
        color: 0x00ff00, // Bright green for mount points
        transparent: true,
        opacity: 1.0,
        emissive: 0x00ff00, // Add glow effect
        emissiveIntensity: 0.5
      })
    );
    ...
    const ring = new THREE.Mesh(
      new THREE.RingGeometry(0.015, 0.02, 32), // Slightly larger ring
      new THREE.MeshBasicMaterial({ 
        color: 0x00aa00, // Darker green for contrast
        transparent: true,
        opacity: 0.9,
        side: THREE.DoubleSide
      })
    ...
    // Add a label with anchor number - larger and more visible
    const labelCanvas = document.createElement('canvas');
    labelCanvas.width = 128; // Higher resolution
    labelCanvas.height = 128; // Higher resolution
    const ctx = labelCanvas.getContext('2d');
    
    // Create a contrasting background for the number
    ctx.fillStyle = '#ffffff'; // White background
    ctx.beginPath();
    ctx.arc(64, 64, 30, 0, Math.PI * 2); // Draw a circle
    ctx.fill();
    
    // Draw the number
    ctx.fillStyle = '#004400'; // Dark green text
    ctx.strokeStyle = '#000000'; // Black outline
    ctx.lineWidth = 3;
    ctx.font = 'bold 60px Arial'; // Larger font
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.strokeText((index + 1).toString(), 64, 64);
    ctx.fillText((index + 1).toString(), 64, 64);
    
    const labelTexture = new THREE.CanvasTexture(labelCanvas);
    const label = new THREE.Mesh(
      new THREE.PlaneGeometry(0.015, 0.015), // Larger label
      new THREE.MeshBasicMaterial({
        map: labelTexture,
        transparent: true,
        depthTest: false
...
function updateStepDisplay() {
  ...
  switch (appState.currentStep) {
    case 0:
      ...
      // Hide Start Over button in first step
      if (appState.ui.startOverButton) {
        appState.ui.startOverButton.visible = false;
      }
      break;
    case 1:
    ...
    case 3:
      ...
      // Show Start Over button in final step
      if (appState.ui.startOverButton) {
        appState.ui.startOverButton.visible = true;
      }
      break;
  }
...
function checkUIHover(controller) {
  ...
  if (appState.ui.startOverButton && appState.ui.startOverButton.visible) buttons.push(appState.ui.startOverButton);
        ...
        if (indexFingerPosition) {
          ...
          if (distance < 0.03) { // 3cm touch distance
            ...
            if (buttonMesh.parent === appState.ui.nextButton) {
              ...
              resetCurrentStep();
            } else if (buttonMesh.parent === appState.ui.startOverButton) {
              // Complete reset functionality (duplicated from onSelectStart)
              appState.currentStep = 0;
