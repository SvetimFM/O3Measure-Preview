...
ViewItemsMenu.render = function() {
  const { width, height, borderColor, params } = this.data;
...
ViewItemsMenu.showCalibrationRequired = function() {
  ...
  const button = this.createButton({
    ...
    position: `0 ${height/2 - 0.115} 0`, // Adjusted vertical position
    width: 0.2,
    height: 0.03,
...
ViewItemsMenu.showObjectGrid = function() {
  ...
  objects.forEach((object, index) => {
    ...
    buttonEl.setAttribute('button', {
      ...
      hoverColor: '#555555',
      pressColor: '#666666',
      label: '', // Empty label as we'll add our own content
      toggleable: false
    });
  ...
  const contentHeight = rows * (itemHeight + itemMargin);
...
ViewItemsMenu.setupScrollingControls = function() {
  ...
  ['left', 'right'].forEach(hand => {
    ...
    this.addEventListener(handEntity, 'triggerdown', (evt) => {
    ...
    this.addEventListener(handEntity, 'pinchstarted', (evt) => {
      // Only process if we have active raycaster intersections
      if (rayEntity.components.raycaster.intersections.length > 0) {
        // Get the intersected element
        const intersectedEl = rayEntity.components.raycaster.intersections[0].object.el;
...
ViewItemsMenu.handleButtonPress = function(action, event) {
