...
AFRAME.registerComponent('wrist-ui', {
  ...
  createPanel: function() {
    ...
    this.panel.setAttribute('slice9', {
      ...
      src: textureSrc,     // Add the texture source here
      usingCustomMaterial: true,  // Use built-in material handling
      transparent: false    
    });
  ...
  applyCustomMaterials: function() {
    ...
    const borderMaterial = new THREE.MeshBasicMaterial({
      map: texture,
      color: new THREE.Color(this.data.borderColor),
      opacity: this.data.opacity,
      transparent: true
    });
    
    // Apply materials
    this.panel.object3D.traverse(function(child) {
      if (child.type === 'Mesh') {
        child.material = panelMaterial;
      }
    });
    
    this.border.object3D.traverse(function(child) {
      if (child.type === 'Mesh') {
  ...
  positionPanel: function() {
    ...
    if (this.data.hand === 'left') {
      ...
      this.el.setAttribute('position', '0.01 111.08 1.08');
