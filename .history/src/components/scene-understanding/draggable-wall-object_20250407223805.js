...
AFRAME.registerComponent('draggable-wall-object', {
  ...
  updateWallReference: function() {
    ...
    if (this.objectRef) {
      ...
      // First create a point at the center of the wall
  ...
  addEventListeners: function() {
    ...
    if (this.wallEntity) {
      this.wallEntity.addEventListener('componentchanged', () => {
        this.updateWallReference();
      });
    }
  },
  
  onRaycastHit: function() {
    // Store the raycaster for later use
    this.raycaster = evt.detail.el.components.raycaster;
  },
  
  onRaycastEnd: function(evt) {
    // End dragging
    if (this.isDragging) {
      this.isDragging = false;
